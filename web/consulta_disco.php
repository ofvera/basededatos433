...
<?php 
    ...
    $query_integrantes_33 = "SELECT a.nombre AS nombre_artista, a.aid AS id_artista
                			 FROM disco d, artista a, banda b, pertenece pe
                			 WHERE pe.bid = "$id_banda" AND a.aid = pe.aid AND pe.fecha_termino IS NULL;";
    //fecha disco
	...
	$query_conciertos_cf_4 = "SELECT C.nombre 
						      ...
						      WHERE ((CB.id_b = "$bid" AND C.id_c = CB.id_c) OR (BI.bid = "$bid" AND C.id_c = BI.id_C)) AND C.fecha <= "$fecha_disco_sig" AND C.fecha >= "$fecha_disco";";

	//conciertos asociados si no hay discos dsp
	$query_conciertos_sf_4 = "SELECT C.nombre 
							  FROM concierto C, concierto_banda CB, banda_invitada BI 
							  WHERE ((CB.id_b = "$bid" AND C.id_c = CB.id_c) OR (BI.bid = "$bid" AND C.id_c = BI.id_C)) AND C.fecha >= "$fecha_disco";";

	//noticias asociadas artistas 
	$query_noticias_art_4 = "SELECT AI.id_a, n.titulo 
							 FROM noticias N, artistas_involucrados AI 
							 WHERE N.id_n = AI.id_n AND N.fecha <= DATEADD(month, 2, "$fecha_disco") AND N.fecha >= "$fecha_disco_sig";";

	//noticias asociadas banda
	$query_noticia_band_4 = "SELECT n.titulo 
							 FROM noticias N, bandas_involucrados BI 
							 WHERE N.id_n = BI.id_n AND BI.id_b = "$bid" AND N.fecha <= DATEADD(month, 2, "$fecha_disco") AND N.fecha >= "$fecha_disco_sig";";
...
if (empty($query_id_banda)) {
	...
	$result -> execute();
	$query_integrantes = $result -> fetch();
	echo "Integrantes";
	...
	$result4 -> execute();
	$query_noticias_band = $result -> fetch();

	//noticias asociadas artistas
	$result4 = $db4 -> prepare($query_noticia_art_4);
	$result4 -> execute();
	$query_noticias_art = $result -> fetch();
	...
	$result -> execute();
	$fecha_disco_33 = $result -> fetch();
	$fecha_disco = $fecha_disco_33[0]
