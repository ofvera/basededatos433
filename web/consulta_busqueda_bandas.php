...
<?php
    ...
    $query4 = "SELECT id_b as b.id_banda, n.titulo as noticia, c.nombre as concierto
               ...
               WHERE b.id_b = bo.id_b AND bo.id_n = n.id_n AND bi.id_b = b.id_b AND bi.id_c = c.id_c AND c.fecha > NOW()
               UNION
    ...
    $query33 = "SELECT b.id as id_banda, null as id_integrante_actual, p.aid as id_integrante_pasado, correo, disco
                ...
                pu.bid = b.id AND pu.did = d.did AND p.fecha_termino < NOW() AND b.nombre = '$nombre' AND n.id_b = b.id
                UNION
                ...
                WHERE b.id = p.bid AND p.aid = a.id AND a.id = contactos.aid AND
                pu.bid = b.id AND pu.did = d.did AND (NOW() BETWEEN p.fecha_inicio AND p.fecha_temrmino)
                ";
